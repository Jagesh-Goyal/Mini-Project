"""
Demand Forecasting & Turnover Prediction Models
-------------------------------------------------
Models:
    - Demand Forecasting  : Random Forest Regressor
    - Turnover Prediction : Gradient Boosting Classifier

Trained models are saved in ml/models/ as .pkl files.
...
class MLModels:
    ...
    def generate_synthetic_training_data(self, db: Session, months: int = 24):
        """
        ...
        # Trending skills receive a 50 % growth simulation
        ...
        for month in range(1, months + 1):
            ...
            for skill_name in skill_names:
                for department in departments:
                    ...
                    trend_boost    = (1.5 ** (month / 24)) if skill_name in trending_skills else 1.0
    ...
    def train_models(self, db: Session) -> dict:
        ...
        try:
            ...
            df["department_encoded"] = self.label_encoders["demand_department"].fit_transform(df["department"])
        ...
        try:
            ...
            for emp in employees:
                ...
                raw_prof    = db.query(EmployeeSkill.proficiency_level).filter(EmployeeSkill.employee_id == emp.id).all()
                avg_prof    = np.mean([p[0] for p in raw_prof]) if raw_prof else 2.5
    ...
    def forecast_demand(
        ...
        """
        Forecast skill demand using the Random Forest model.

        Args:
            skill_name : Skill to forecast for.
            months_ahead: 3, 6, or 12 months.
            department : Optional — if None, forecast across all departments.
            db         : DB session for supply lookup.
        """
        ...
        try:
            ...
            department_encoder = self.label_encoders.get("demand_department") or self.label_encoders.get("department")

            if skill_encoder is None or department_encoder is None:
                return {"error": "Model encoders missing. Please retrain model."}

            if skill_name not in set(skill_encoder.classes_):
                return {"error": "Skill not in trained model. Retrain after adding this skill."}

            # Resolve departments to forecast over
            if department is None:
                employees      = db.query(Employee).all() if db else []
                db_departments = sorted({e.department for e in employees if e.department})
                departments    = [d for d in db_departments if d in set(department_encoder.classes_)]
                if not departments:
                    departments = list(department_encoder.classes_)
            else:
                if department not in set(department_encoder.classes_):
                    return {"error": "Department not in trained model. Retrain after adding department data."}
            ...
            base_supply = (
                db.query(EmployeeSkill).filter(EmployeeSkill.skill_id == skill_obj.id).count()
                if db and skill_obj else 5
            )
            ...
            for dept in departments:
                ...
                for month_offset in range(1, months_ahead + 1):
                    ...
                    gap = max(0, int(demand_pred) - base_supply)
            ...
            return {"skill": skill_name, "months_ahead": months_ahead, "forecasts": forecast_result}
    ...
    def predict_turnover_risk(self, employee_id: int, db: Session) -> dict:
        ...
        try:
            ...
            department_encoder = (
                self.label_encoders.get("turnover_department")
                or self.label_encoders.get("department")
            )
            if department_encoder is None:
                return {"error": "Turnover encoder missing. Please retrain model."}

            emp_dept = emp.department or "Unknown"
            if emp_dept not in set(department_encoder.classes_):
                emp_dept = department_encoder.classes_[0]

            dept_code = department_encoder.transform([emp_dept])[0]
